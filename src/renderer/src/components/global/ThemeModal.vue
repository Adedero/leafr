...
const filteredSections = computed(() => {
  ...
  return (["light", "dark"] as const).map((section) => ({
    section,
    themes: theme[`${section}Themes`].filter((t) =>
  ...
  <Modal title="Theme" @close="search = ''">
    ...
    <template #body>
      <div class="space-y-10">
        <div
          v-for="{ section, themes } in filteredSections"
          :key="section"
          class="space-y-4"
        >
          ...
          <div
            v-else
            class="items-start gap-2 grid grid-cols-[repeat(auto-fill,minmax(5rem,1fr))]"
          >
            <button
              v-for="t in themes"
              :key="t.name"
